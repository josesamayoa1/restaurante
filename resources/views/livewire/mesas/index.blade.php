    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                ...
                <p class="mt-2 text-sm text-gray-700">Lista de todas las mesas del sistema con sus nombres y acciones disponibles.</p>
            </div>
        ...
        <!-- Buscador -->
        <div class="mt-4">
            <input wire:model.debounce.300ms="search" type="text" placeholder="Buscar mesas..."
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($mesas as $mesa)
                                <tr>
                                    <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-0">{{ $mesa->id }}</td>
                                    <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ $mesa->nombre }}</td>
                                    <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-0">
                                        <button wire:click="edit({{ $mesa->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                            Editar<span class="sr-only">, {{ $mesa->nombre }}</span>
                                        </button>
                                        <button wire:click="confirmDelete({{ $mesa->id }})" class="text-red-600 hover:text-red-900">
                                            Eliminar<span class="sr-only">, {{ $mesa->nombre }}</span>
                                        </button>
                                    </td>
                                </tr>
                            @empty
                        ...
                        <div>
                            ...
                            <div class="mt-3 text-center sm:mt-5">
                                <h3 class="text-base font-semibold text-gray-900" id="modal-title">
                                    {{ $mesa_id ? 'Editar Mesa' : 'Crear Nueva Mesa' }}
                                </h3>
                                <div class="mt-2">
                                    <div>
                                        <label for="nombre" class="block text-sm/6 font-medium text-gray-900">Nombre de la Mesa</label>
                                        <div class="mt-2">
                    ...
                    <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
                        <div>
                            ...
                            <div class="mt-3 text-center sm:mt-5">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">¿Estás seguro de que deseas eliminar esta mesa? Esta acción no se puede deshacer.</p>
                                </div>
                            </div>
                        </div>
                        <div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
