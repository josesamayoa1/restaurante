    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                ...
                <p class="mt-2 text-sm text-gray-700">Lista de todos los roles del sistema con sus nombres y acciones disponibles.</p>
            </div>
        ...
        <!-- Buscador -->
        <div class="mt-4">
            <input wire:model.debounce.300ms="search" type="text" placeholder="Buscar roles..."
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($roles as $role)
                                <tr>
                                    <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-0">{{ $role->id }}</td>
                                    <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ $role->name }}</td>
                                    <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-0">
                                        <button wire:click="edit({{ $role->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                            Editar<span class="sr-only">, {{ $role->name }}</span>
                                        </button>
                                        <button wire:click="confirmDelete({{ $role->id }})" class="text-red-600 hover:text-red-900">
                                            Eliminar<span class="sr-only">, {{ $role->name }}</span>
                                        </button>
                                    </td>
                                </tr>
                            @empty
                        ...
                        <div>
                            ...
                            <div class="mt-3 text-center sm:mt-5">
                                <h3 class="text-base font-semibold text-gray-900" id="modal-title">
                                    {{ $role_id ? 'Editar Rol' : 'Crear Nuevo Rol' }}
                                </h3>
                                <div class="mt-2">
                                    <div>
                                        <label for="nombre" class="block text-sm/6 font-medium text-gray-900">Nombre del Rol</label>
                                        <div class="mt-2">
                    ...
                    <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
                        <div>
                            ...
                            <div class="mt-3 text-center sm:mt-5">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">¿Estás seguro de que deseas eliminar este rol? Esta acción no se puede deshacer.</p>
                                </div>
                            </div>
                        </div>
                        <div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
